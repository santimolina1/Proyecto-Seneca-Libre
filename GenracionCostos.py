...
# Datos ya cargados
vehicles = pd.read_csv('case_1_base/Vehicles.csv')  # Vehículos
vehicles['ID'] = ['V' + str(i + 1) for i in range(len(vehicles))]  # Generar IDs para vehículos
distancias = pd.read_csv('Matrices/distancias1.csv')  # Matriz de distancias
tiempos = pd.read_csv('Matrices/tiempos1.csv')  # Matriz de tiempos
